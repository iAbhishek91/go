...
func main() {
	...
	( *s).print() // Output: {10 London}

	( *s).setValue(20, "UK") // Output {20 UK}

	( *s).print() // Output: {10 London} | value will not change automatically as we are using value receiver.

	*s = ( *s).setValueNReturn(30, "UK") 

	( *s).print() // Output: {30 UK} | as we can user need to manually need to apply the change on the type.
}